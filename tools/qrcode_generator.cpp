...
struct State {
    ...
    State()
        : text("Hello, QR!"), version(0), eclIdx(1), scale(8), quietZone(4), showHelp(false) {}
};
...
static void overlayInfo(Mat& canvas, const State& s) {
    ...
    put(format("Scale: %d (+/-)  QuietZone: %d ([/])", s.scale, s.quietZone));
    ...
    if (s.showHelp) {
        ...
        put("  +/- scale, [/ ] quiet zone", Scalar(200,200,200));
...
int main(int argc, char** argv) {
    ...
    for (;;) {
        ...
        if (ch == '+') { s.scale = clampi(s.scale + 1, 1, 64); needRedraw = true; continue; }
        if (ch == '-') { s.scale = clampi(s.scale - 1, 1, 64); needRedraw = true; continue; }
        if (ch == '[') { s.quietZone = clampi(s.quietZone - 1, 0, 16); needRedraw = true; continue; }
        if (ch == ']') { s.quietZone = clampi(s.quietZone + 1, 0, 16); needRedraw = true; continue; }
        ...
        if (isChar && ch >= 32 && ch <= 126) { s.text.push_back(static_cast<char>(ch)); needRedraw = true; continue; }
        // Backspace (8 or 127)
        if (key == 8 || key == 127) { if (!s.text.empty()) { s.text.pop_back(); needRedraw = true; } continue; }
    }
